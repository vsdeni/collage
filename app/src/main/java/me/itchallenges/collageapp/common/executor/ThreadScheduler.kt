...
constructor() : ExecutionScheduler {
    ...
    override fun <T> highPriorityMaybe() = { upstream: Maybe<T> -> upstream.subscribeOn(highPriority()).observeOn(ui()) }

    override fun <T> lowPriorityMaybe() = { upstream: Maybe<T> -> upstream.subscribeOn(highPriority()).observeOn(ui()) }
